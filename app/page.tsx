...
function buildFollowUp(d: AlertData): string {
  ...
  if (alertKey.includes("application usage")) {
    ...
    body = lines(["IDS Signature", getNested("ids.signature"), "", "Source Host", get("srcip_host"), "", "Destination IP", get("dstip"), "", "Destination Host", get("dstip_host"), "", "IDS Action", getNested("ids.action"), "", "Could you confirm if this is expected on your end? Thank you."]);
  } else if (alertKey.includes("internal account login failure")) {
    body = lines(["Source Username", get("srcip_username"), "", "Total Number Failed", getNested("event_summary.total_failed") !== "N/A" ? getNested("event_summary.total_failed") : get("actual"), "", "Total Number Successful", getNested("event_summary.total_successful") !== "N/A" ? getNested("event_summary.total_successful") : get("num_successful"), "", "Login Type", get("login_type"), "", "Source IP", get("srcip_host"), "", "Please confirm whether these login failure attempts are legitimate or not. Thank you!"]);
  } else if (alertKey.includes("external firewall policy")) {
    body = lines(["Source IP", get("srcip_host"), "", "Source Port", get("srcport"), "", "Destination IP", get("dstip_host", "dstip"), "", "Destination Port", get("dstport"), "", "Device Type", get("dev_type"), "", "Firewall Policy", get("fw_policy_id"), "", "Please verify if this firewall policy activity is part of your operations. Thank you."]);
  } else if (alertKey.includes("external firewall denial")) {
    body = lines(["Source IP", get("srcip_host"), "", "Destination IP", get("dstip_host", "dstip"), "", "Action", get("action"), "", "Please verify the source IP if related to your operations, Thank you!"]);
  } else if (alertKey.includes("encrypted phishing")) {
    body = lines(["Source IP", get("srcip_host"), "", "Destination IP", get("dstip"), "", "Destination Host", get("dstip_host"), "", "Destination Country", getGeo("dstip"), "", "Please confirm if this activity is expected or legitimate. If not, we recommend blocking the destination host and avoiding access to this site. Thank you."]);
    ...
    body = lines(["Source IP", get("srcip_host"), "", "Destination IP", get("dstip_host", "dstip"), "", "IDS Signature", getNested("ids.signature"), "", "Source Port", get("srcport"), "", "Destination Port", get("dstport"), "", "We detected suspicious internal traffic that matched a known malware IDS signature. Kindly verify the affected endpoint and confirm whether this activity is expected. Thank you!"]);
    ...
    body = lines(["Source IP", `${get("srcip_host", "host_ip", "IP/name", "ip")} (Malicious)`, "", "Source Country", getGeo("srcip"), "", "Event Source", get("event_source"), "", "action", get("action"), "", "Please verify the source IP if related to your operations, as it was flagged malicious by security vendors. Thank you!"]);
